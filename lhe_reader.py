...
class lhe_reader(object):
    def __init__(self, lhefile) -> None:
        ...
        LHE_analyzer : object
            This is some sort of class that is used to look at events that you are using
            This class should work like the ones in lhefile.py - i.e. have mothers, daughters, and associated as fields
    ...
    def cross_section(self):
        ...
        head = self.head #The part before the events contains the cross section
    ...
    @functools.cached_property
    def head(self):
        return self.text[:self.text.find("<event>")] #everything until the first event
    
    @functools.cached_property
    def tail(self):
        return self.text[self.text.rfind("</event>") + len("</event>"):] #everything after the last event
    ...
    def cut_down_to_size(self, n, verbose=False, shuffled=False, dump=""):
        ...
        ValueError
            n must be > 0
        """
        start_of_file, end_of_file = self.head, self.tail
        ...
        cut_down = random.sample(self.all_events, n) if shuffled else self.all_events[:n]
